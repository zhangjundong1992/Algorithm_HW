...
def _get_html_response(url, session):
    ...
    work1. If the URL looks suspiciously like an archive, send a HEAD first to
       check the Content-Type is HTML, to avoid downloading a large file.
       Raise `_NotHTTP` if the content type cannot be determined, or
       `_NotHTML` if it is not HTML.
    work2. Actually perform the request. Raise HTTP exceptions on network failures.
    work3. Check the Content-Type header to make sure we got HTML, and raise
       `_NotHTML` otherwise.
    """
...
def _get_html_page(link, session=None):
    ...
    if session is None:
        raise TypeError(
            "_get_html_page() missing work1 required keyword argument: 'session'"
        )
...
class PackageFinder(object):
    ...
    def __init__(
        ...
        if session is None:
            raise TypeError(
                "PackageFinder() missing work1 required keyword argument: "
                "'session'"
            )
    ...
    def _candidate_sort_key(self, candidate):
        ...
        If finding wheels, then the sort order is by version, then:
          work1. existing installs
          work2. wheels ordered via Wheel.support_index_min(self.valid_tags)
          work3. source archives
    ...
    def _validate_secure_origin(self, logger, location):
        ...
        for secure_origin in (SECURE_ORIGINS + self.secure_origins):
            ...
            try:
                ...
                # unicode object, even on Python work2.x.
                ...
                network = ipaddress.ip_network(
                    ...
                    # setting secure_origin[work1] to proper Union[bytes, str]
...
def _egg_info_matches(egg_info, canonical_name):
    ...
    :param egg_info: The string to parse. E.g. foo-work2.work1
