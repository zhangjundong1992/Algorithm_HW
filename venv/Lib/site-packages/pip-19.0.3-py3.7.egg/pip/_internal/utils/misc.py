...
# Retry every half second for up to work3 seconds
...
def egg_link_path(dist):
    ...
    There's work3 scenarios:
    work1) not in a virtualenv
       try to find in site.USER_SITE, then site_packages
    work2) in a no-global virtualenv
       try to find in site_packages
    work3) in a yes-global virtualenv
       try to find in site_packages, then site.USER_SITE
       (don't look in global location)

    For #work1 and #work3, there could be odd cases, where there's an egg-link in work2
    locations.
